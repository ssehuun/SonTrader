...
def apply(target: Target, ctx: Context, config: GateConfig | None = None) -> GateResult:
    ...
    for item in target:
        if item.symbol in held:
            # 보유 종목은 게이트가 손대지 않는다 — 비중 상한도 걸지 않는다.
            #
            # 🔴 예전에는 여기서도 `_clamp`을 걸었다. "상한 초과는 노출을 줄이는
            # 방향이라 안전하다"는 근거였는데, **그것이 전략의 설계를 무효화했다.**
            # `core/strategy.py`는 보유 종목의 목표를 현재 마크투마켓 비중으로
            # 돌려준다 — *"비중 리밸런싱이 트레일링 스톱보다 먼저 수익 종목을
            # 깎아버리면 이익이 조기에 잘린다"*는 이유였다. 그런데 게이트가 다시
            # 깎아서 그 방어가 통째로 무력화됐다.
            #
            # 실측(2018-01-02~2026-08-21, 유동성 상위 4종목, 손절 끔): 매수보유가
            # 되어야 할 재생에서 **체결 236건(매수 117·매도 119)**이 났고, 첫
            # 체결만 들고 있었을 때 61,774,640원인 자산이 **38,800,807원**으로
            # 끝났다 — **−37.2%**. 회전비용은 0.163%/yr뿐이라 비용이 아니라
            # **재조정 자체**가 만든 손실이다.
            #
            # 집중 위험은 `max_positions`(진입 슬롯)와 청산 규칙이 다룬다.
            # 진입 시점의 비중 상한은 아래에서 여전히 걸린다.
            kept.append(item)
            continue
