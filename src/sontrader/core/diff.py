...
**종가 동시호가 집행은 이 상한을 받지 않는다** — `_margin_unit_price` 참고.
...
def _margin_unit_price(base_price: int, price_limit: float) -> int:
    ...
    **거래비용.** 시가가 정확히 상한가인 날(상한가 시초가)에는 비용만큼 모자라
    ...
    ## 종가 동시호가 집행에서는 이 배수가 필요 없다

    1.30은 **체결가를 주문 시점에 모르기 때문에** 무는 값이다. 종가 동시호가로
    집행하면(`SimBrokerConfig.fill_at_close`) 수량 환산에 쓴 종가가 곧 체결가라
    상한가만큼 잡아둘 이유가 없다 — 그때는 `price_limit`을 슬리피지·수수료를
    덮을 만큼만(예: 0.002) 준다. 손잡이가 아니라 **집행 방식이 정하는 값**이다.
