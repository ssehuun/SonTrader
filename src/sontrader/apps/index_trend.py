"""지수 이동평균 추세 필터 (S1) — **하네스 교정용**.

## 이건 전략 채택 후보가 아니다

우리는 백테스트를 **알려진 정답으로 맞춰본 적이 한 번도 없다.** 지금까지 잰
것은 전부 "이 전략이 지는가"였고 전부 졌는데, **하네스가 고장 나 있어도
...
이평을 구해 수익률을 내는 것은 짧은 스크립트로 되지만 **그건 하네스를 안
...
**하네스의 일부만 검정한다.** ETF 일봉이 DB에 없어(`069500`·`102110` 둘 다
0행) **지수로 재생**한다.
