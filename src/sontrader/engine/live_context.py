...
안전한 이유는 이미 두 겹의 멱등성이 있기 때문이다 — `core/gate.py`가
이미 진입한 이벤트(`used_event_ids`)를 막고, `engine/approval.py`의
`propose()`가 이미 대기 중인 제안을 event_id 기준으로 중복 생성하지
않는다. 재조회 비용은 이 규모(워치리스트 ≤50종목)에서 무시할 만하다.
