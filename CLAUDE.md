...
## 훅이 강제하는 것 — `.claude/settings.json`

| 훅 | 하는 일 |
|---|---|
| `scripts/bash_guard.py` 검사 1 | **`.env`·토큰 캐시의 시크릿 읽기를 차단.** 2026-08-20에 실전 앱키가 대화 기록에 유출돼 재발급한 적이 있다 (`01-실전-차단.md` T3). 값 대신 **변수명만** 본다: `grep -oE '^KIS_[A-Z_]+=' .env` |
| 검사 2 | `KIS_PAPER=false`(실전)에서 KIS 호출 — **주문 경로는 차단**, 조회는 확인 |
| 검사 3 | `index-trend`·`backtest`의 `--start`/`--end`/`--code`를 등록창과 대조 |
| ruff (`PostToolUse`) | `.py` 편집 직후 `check --fix` + `format` |

**등록창을 바꿀 때는 규약 §5.1·§5.1b와 스크립트의 `REGISTERED`를 함께 고친다.**
훅이 못 잡는 것: 체결 시점·비용·반증 조건·판정 기준 지표·비교 기준 — 명령줄에
안 나타난다. 그건 여전히 규율에 맡긴다.
...
- 가정을 말한 뒤 진행한다. 확신이 없으면 묻는다. **해석이 여럿이면
  조용히 하나를 고르지 않고 갈래를 제시한다.**
- 더 단순한 방법이 있으면 말한다. 근거가 있으면 **되민다** —
  동의만 하는 것은 도움이 아니다.
- 막히면 멈춘다. 무엇이 불분명한지 이름을 붙여 묻는다.

## 최소로 만든다

- 요청 범위 밖의 기능, 한 곳에서만 쓰는 추상화, 요청하지 않은
  설정 가능성을 만들지 않는다.
- 일어날 수 없는 경우에 에러 처리를 넣지 않는다.
- 200줄이 50줄로 되면 다시 쓴다.

## 개발·검증 절차

1. 작업 범위를 하나의 기능 단위로 쪼개고, 관련 문서의 완료 기준을 확인한다.
2. 실제 코드·테스트·최근 이력을 읽은 뒤 최소 변경으로 구현한다. **바꾼 줄마다 요청과의 연결이 보여야 한다.**
   - 인접한 코드·주석·형식을 "개선"하지 않는다. 기존 스타일에 맞춘다 — 내가 다르게 쓸 것이어도.
   - 내 변경이 만든 고아(import·변수·함수)만 치운다. **원래 있던 죽은 코드는 말하되 지우지 않는다.**
