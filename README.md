...
- `todo/` — 미룬 작업. `01-실전-차단.md`가 실전 운영을 막는 것들이고,
  나머지는 단순한 매매가 안정된 뒤에 손댄다.
- `docs/scenario/` — 매크로 판단 시스템. 국면 판단을 붙이기 위한 설계와,
  **가설을 사전 등록하고 하나씩 반증해 기록하는 프로토콜**(`00` 설계서 3절).
  지표 카탈로그(`01`), 가설 원장(`02`, append-only), 트레이딩 시나리오(`03`).
